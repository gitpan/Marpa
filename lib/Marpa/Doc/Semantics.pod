...
Marpa::API::Semantics - How Marpa Evaluates Parses
...
Some rules are L<sequence rules|Marpa::API::Grammar/"Sequence Rules">.
...
A null node gets its value at parse setup time from the null
value of a symbol.
If the null node is for a nulled symbol,
the node's value is the null value of the nulled symbol.
If the null node is for a nulled rule,
the node's value is the null value of the rule's left
hand side symbol.

If no C<null_value> property is defined for a symbol,
that symbol's null value is
the default null value for
the grammar.
The default null value is a Perl C<undef>.
A grammar's
default null value can be changed by defining the
L<C<default_null_value> named argument|Marpa::API::Grammar/default_null_value>.
