...
In Marpa, all symbols are terminals are default.
...
Here is a complete listing of the ways in which a symbol C<X> can become a terminal:

=over

=item * A symbol C<X> is a terminal by default.

=item * If no symbols are directly marked as terminals, and
the L<C<lhs_terminals> named argument|/"lhs_terminals"> is set,
symbol C<X> is a terminal.
These are the defaults, so this case simply restates the previous one.

=item * If symbol C<X> is directly marked using the L<C<terminals> named argument|/"terminals">,
it is a terminal.

=item * If symbol C<X> is directly marked using the L<C<terminal> property|/"terminal">,
it is a terminal.

=item * If symbol C<X> is not the LHS symbol for any rule,
the L<C<lhs_terminals> named argument|/"lhs_terminals"> is unset,
and no terminals are marked directly,
then symbol C<X> is implicitly marked as a terminal.
...
If the LHS of an empty rule could also be a terminal,
it would make life difficult for the recognizer.
...
with the LR(0) items, LR(0) NFA states, and transitions for each.
