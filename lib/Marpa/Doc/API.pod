...
It is the starting point for reading the
API documentation.

The two examples in the L<synopsis|SYNOPSIS>
show the typical flow of calls in the Marpa API.
...
can be found in L<the other Marpa API documents|OTHER API DOCUMENTS>.
...
In Marpa these are three phases.
These phases are for the most part distinct, and
the Marpa API is structured around them.

=head2 Creating a Grammar

The package L<Marpa::API::Grammar> creates grammars.
...
The L<Marpa::API::Recognizer> package creates recognizers from grammars.
...
a Marpa application
can specify a Perl closure to implement
its semantics.
For each token,
...
the parse trees.
The value of the root node of a parse tree is the value of the parse
...
Marpa grammars are L<Marpa::API::Grammar> objects.
They are created
with the
L<Marpa::Grammar::new|Marpa::API::Grammar/"new">
constructor.
The arguments to 
L<Marpa::Grammar::new|Marpa::API::Grammar/"new">
are references to
...
rely for their interpretation
on the evaluators.

The evaluators will be described
L<later|/"THE SINGLE PARSE EVALUATOR">.
Peeking ahead,
C<actions> is the name
of a Perl package where Marpa looks for
the semantic Perl closures.
C<default_action> is the action name.
The C<default_action> action name will
resolve to a Perl closure
...
In the example, its argument is a reference to an array of B<token descriptors>.
...
In these examples, the per-parse object is not used.
...
the per-parse object will be its only argument.
...
L<Marpa::API::Recognizer>'s Single Parse Evaluator is far more powerful
...
in which you cannot use Marpa's Single Parse Evaluator.
The Multi-parse Evaluator handles all possible general parses --
every possible input to any BNF-expressible grammar.
...
for L<Marpa::API::Evaluator>.

Finitely ambiguous grammars
can be very useful.
...
from finitely ambiguous grammars.
...
L<the Multi-parse Evaluator|Marpa::API::Evaluator>
is the right tool.
...
but it also means there can be multiple parses,
and the different parses will often have different values.
In this application we decide, for each input,
to return the value for every one of its parses.
...
It returns a reference to the value of the current parse.
...
=head1 OTHER API DOCUMENTS

This document gave a semi-tutorial overview of the entire Marpa API.
For detail about Marpa's grammar objects, see the
L<Marpa::API::Grammar> document.
For detail about Marpa's recognizer objects, see the
L<Marpa::API::Recognizer> document.

Marpa's standard semantics are fully described in the
L<Marpa::API::Semantics> document.
Techniques for tracing and for debugging your Marpa grammars
are described in the
L<Marpa::API::Tracing> document.

=head2 Documents Yet to be Unwritten

The L<Marpa::API::Implementation> document will describe Marpa's internals,
focusing on the Marpa tables (earley sets) and how to read them.
This information can be quite useful for users of Marpa who are
tracing and debugging grammars they've written.

Two other as-yet-unwritten documents desribe advanced features.
The L<Marpa::API::Evaluator> document will give details about the Multi-parse Evaluator.
The L<Marpa::API::Models> document will give details about alternative models
