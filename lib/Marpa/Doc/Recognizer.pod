...
A lot of what
L<Marpa::Recognizer> does can be
regarded as "evaluation".

In particular,
L<Marpa::Recognizer> has a built-in
...
The C<status> method is the only way to get the list of
the expected terminals at earleme 0, before any tokens
have been recognized.
It can also been used whenever it is convenient to
liberate checks of the status of the recognizer
from calls to the C<tokens> method.
...
For debugging grammars, try C<show_progress> first.
C<show_earley_sets> 
requires knowledge of Marpa internals to interpret,
and C<show_progress> will usually contain all the detail
necessary to debug grammars
and interpret parse progress.
...
and the string contains reports for earleme I<N>.
...
"-I<M>", the range is to the I<M>th earleme from the last.
In other words,
...
C<show_progress> is
a powerful tool for users who need
to debug application grammars.
...
A trace level of 1 or higher,
traces terminals as they are accepted or rejected
by the recognizer.
A trace level of 2 or higher
traces the terminals expected at every
earleme.
Practical grammars often a large number of different
...
When a parse's ambiguity is deliberate,
C<max_parses> 
can be used when
