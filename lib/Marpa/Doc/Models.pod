...
This means that in Marpa, a parse can be ambiguous,
even when the grammar is not ambiguous.

Marpa's semantics deal
with ambiguity due to lexing in the
same way that Marpa deals with
