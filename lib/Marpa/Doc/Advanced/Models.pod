...
Marpa::Advanced::Models - Other Input Models (Advanced)
...
it defaults to 1.
That means the next token will be added at another
earleme location, one after the current one.
...
It's a bit of a pedantic point, but in the rest of
this documentation I follow the standard
terminology in saying that grammar is ambiguous
if it can produce more than one parse result,
and vice versa.
In Marpa this is not strictly true.
An unambiguous grammar can produce than one parse
in Marpa, if the lexing is ambiguous.
Conversely, if Marpa returns more than
one parse result for a grammar, you cannot conclude
that the grammar itself is ambiguous.
