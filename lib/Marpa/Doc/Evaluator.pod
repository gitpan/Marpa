...
Marpa::Evaluator objects implement Marpa's Multi-parse
Evaluator.
...
L<Marpa::API/"THE SINGLE PARSE EVALUATOR">.
...
is specified with
L<Marpa::Evaluator>'s L</parse_order> named argument.
...
in the L<evaluator setup phase|Marpa::API::Semantics/"Evaluator Setup Phase">,
...
detail in
L<the semantics document|Marpa::API::Semantics/"RESOLVING ACTION NAMES">.

The ranking closures for token leaf nodes
and the ranking closures for nulled leaf nodes
are called
in the L<bocage setup phase|Marpa::API::Semantics/"Bocage Setup Phase">.
The ranking closures for rule nodes are called
during the L<parse setup phase|Marpa::API::Semantics/"Parse Setup Phase">.
...
in a L<separate document|Marpa::API::Bocage>,
...
The C<recce> named argument is not allowed for the L<Marpa::Evaluator::set> call.
...
in the literature, because all cycles yield potentially infinite
parses.
I use the more verbose terms because
most readers will not be familiar with parsing theory,
and might confuse cycles with recursion of the non-pathological
and non-infinite variety.
...
argument|Marpa::API::Grammar/"infinite_action">
