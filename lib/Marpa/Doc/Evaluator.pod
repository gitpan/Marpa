...
This interface is now deprecated in favor of the 3rd Generation
evaluator
which is part of the <Marpa Recognizer|Marpa::Recognizer>.
It may be removed in the future
and, when XS support is added to Marpa.

The 3rd Generation interface benefits
from the design lessons learned from the previous two
generations.  It is faster, more powerful
...
=head1 CONVERTING TO THE 3rd GENERATION EVALUATOR
...
In the 3rd Generation evaluator, it is 'none'.
This means that the C<parse_order> named argument of
the <Marpa Recognizer|Marpa::Recognizer> must be set to
a value other than "C<none>",
or all ranking actions will be silently ignored.

The ranking actions must return a B<reference> to the rank,
not the rank itself.
This change from the 2nd Generation is to allow the ranking
actions to specify that a choice is to be ignored by
...
detail in
L<the semantics document|Marpa::Semantics/"RESOLVING ACTION NAMES">.

The ranking closures for token leaf nodes
and the ranking closures for nulled leaf nodes
are called
in the L<bocage setup phase|Marpa::Semantics/"Bocage Setup Phase">.
The ranking closures for rule nodes are called
during the L<parse setup phase|Marpa::Semantics/"Parse Setup Phase">.
