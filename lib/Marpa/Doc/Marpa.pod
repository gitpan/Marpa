...
Marpa's evaluators find the parse results
in the recognizer's tables.
If the input did not match the grammar,
the tables will contain no parse results,
and the evaluators will indicate that.
If the input did match the grammar,
the tables will contain one or more parse results.
There may be more than one parse result,
...
the parse trees.
The value of the root node of a parse tree
...
For example, for the first rule in the above display,
...
Marpa will look for Perl closures named
...
In most cases
the parse result returned by
Marpa's Single Parse Evaluator is all that is wanted.
...
for which you could
list the parse results forever,
with none of the parse results being duplicates.

=item * The L</"Marpa::Recognizer::value"> method cannot be used if
your parse is ambiguous B<and>
you want to see B<two or more> of the possible parse results.

=item * The L</"Marpa::Recognizer::value"> method cannot be used if
your parse is ambiguous B<and>
you want to be able to B<choose which of the parse results> you see.
...
The Multi-parse Evaluator handles all possible parses --
every possible input to any BNF-expressible grammar.
For unambiguous and finitely ambiguous grammars,
the Multi-parse Evaluator will iterate all the parse results.
The Multi-parse Evaluator also
allows the application to
control the order in which parse results are produced.

For infinitely ambiguous grammars,
the Multi-parse Evaluator will iterate all the non-cyclical parse results.
...
is limited to returning one parse result, chosen arbitrarily.
...
the evaluator will find its parse results.

It is possible that C<Marpa::Evaluator::new> will be able to see
immediately that there are no parse results in the recognizer's tables.
In that case
...
The 
C<Marpa::Evaluator::value> method iterates through the parse results.
It returns a reference to the value of the current parse result.
...
L<Marpa::Tracing> document.

The L<Marpa::Implementation> describes Marpa's internals,
...
The L<Marpa::Evaluator> document provides
details about the Multi-parse Evaluator.
The L<Marpa::Models> document (yet to be written)
...
My academic sources are described in detail in 
L<my bibliography|Marpa::Bibliography> and
L<my (very difficult and sketchy) description of the Marpa algorithm|Marpa::Algorithm>.
Nonetheless, here I am to name those who
created the algorithms which are the direct
and near-direct predecesssors of Marpa --
Jay Earley,
Joop Leo,
John Aycock and
R.  Nigel Horspool.
Without their work Marpa would not exist.
