...
The names of the new symbols are the name of the original left hand side with
two suffixed tags.
...
Rule 0 is a worst case for CHAF, because the last three symbols of the right hand side are all
proper nullables.
...
just as the proper nullables
found in the original rule
had to be split and factored.
...
The user's semantics are preserved, because Marpa,
...
to the user's semantic actions
...
is not B<parsing in the strict sense>.
...
We will use both both transitions in our examples.
...
on an C<Add> token with a token length of 1.
...
Prediction states exist when
the presence of LR(0) items at a certain location
gives rise to the expectation of other LR(0)
items at that same location.
When
an LR(0) item with a post-dot symbol is present at a given
location,
we call that symbol an B<expected symbol>.
For every expected symbol at a given location,
we can also expect
...
In other words, LR(0) item expectations recurse.
...
We'll examine the recursive sequence of LR(0) item
expectations that originates in S8
...
The paired prediction QDFA state for S8 will contain
all the LR(0) item expectations caused by the expectation
of the C<Term> symbol.

Expectation of the C<Term> symbols
gives rise to the expectation of rule
initializations for all rules with
C<Term> on their left hand side.
There are two such rules
and their rule initializations are
...
but C<Number> is new as an expected symbol.

We need to add all rule initializations for any rules
that the C<Number> on the left hand side.
But there is no rule in our grammar with
C<Number> on its left hand side.
So there are no new LR(0) item expectations
to add and our recursion is finished.

Starting with the expected symbols in QDFA state S8,
we found four new LR(0) item expectations.
...
We see that it does contain all four of the LR(0)
item expectations that we found in this section.
...
An Earley item containing a completed rule is called a B<completion cause Earley item>,
or simply a B<cause>.
As a result of recognizing a completion cause Earley item,
Marpa may add
...
as its completion cause.
The origin of a completion effect is always the
same as the origin of its matching predecessor.
The means our new 
...
One more completion cause-effect pair and we will see
S5@0-3 again.
...
it picks just one of the alternatives,
...
These two evaluators create the tree differently.
In the case of an unambiguous parse the two evaluators will produce
exactly the same tree.

Marpa evaluates the pre-order tree using
an evaluation stack.
...
and declarations, and can be extremely long.
A Perl lexical unit is also a sequence.

In the case of sequences written as BNF rules,
the pieces of the sequence
often have no real semantics of their own.
This means that popping and pushing the symbols for every
piece of the sequence is pointless.
It is most efficient to leave the items
in the sequence
on the stack until the sequence is complete.
Often the semantics of individual pieces of the sequence
are a no-op, and
the stack can be left untouched.

Sequences are not the only case where symbols
are best left on the stack.
To optimize these cases,
Marpa marks some of its internal symbols as "virtual".
A symbol is B<virtual> if it has no "real" semantics.
When a symbol is virtual,
Marpa will not bother to do the usual pop and/or push
of symbols on the evaluation stack.

Marpa uses these three
